...
//            NSLog(@"url:%@",url);
            ...
            [imageView sd_setImageWithURL:[NSURL URLWithString:url] placeholderImage:[UIImage imageNamed:@"img_banner_loading"] options:SDWebImageRefreshCached];
            
        }else{
            
//            NSLog(@"cccccc:%ld",self.images.count);
            
//            imageView.image = [UIImage imageNamed:@"img_banner_loading"];
...
- (void)changeScrollOffset {
    ...
    [self.bannerScrollView setContentOffset:CGPointMake(self.frame.size.width  * (page + 1), 0) animated:YES];
