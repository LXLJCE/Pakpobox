...
- (void)setupNavigationBar {
    self.navigationItem.title = PaxScan;
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:PaxAlbum style:(UIBarButtonItemStyleDone) target:self action:@selector(rightBarButtonItenAction)];
...
//        [SGQRCodeNotificationCenter postNotificationName:SGQRCodeInformationFromeScanning object:obj.stringValue];
        NSLog(@"%@", obj.stringValue); // 是我想要的
        
        [self networkRequestWith:obj.stringValue];
    }
}
...
/**
 ...
 @param str 扫描结果
 */
- (void)networkRequestWith:(NSString *)str
{
    
    NSString *time = [NSString stringWithFormat:@"%.0f",[NSDate date].timeIntervalSince1970 * 1000];
    [[LXLNetWorkTool shareTools]Pax_ScanToUnlockWithTipMessage:PaxPleasewaitamomentLoad qrcode:str timestamp:time Finish:^(id responseObject, NSError *error) {
