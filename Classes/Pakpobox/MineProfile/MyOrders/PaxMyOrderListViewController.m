//
//  PaxMyOrderListViewController.m
//  Pakpobox
//
...
@interface PaxMyOrderListViewController ()<UITableViewDataSource,UITableViewDelegate>
...
@implementation PaxMyOrderListViewController
- (NSMutableArray *)arr
{
    if(_arr == nil){
...
- (void)setupTableview
{
    _tableView = [[UITableView alloc] initWithFrame:CGRectMake(16, 100, self.view.bounds.size.width- 2 * 16, self.view.bounds.size.height - 100) style:UITableViewStylePlain];
...
{
    LXLWeakSelf(self);
    [[LXLNetWorkTool shareTools] Pax_GetMyOrdersWithTipMessage:PaxPleasewaitamomentLoad orderType:@"" page:self.currentPage maxCount:self.maxCount Finish:^(id responseObject, NSError *error) {
        NSArray *list = responseObject[@"resultList"];
        if (list == 0) {
            [PaxHUD showSuccessWithStatus:PaxSorrnodatayouwant];
        }else{
            [PaxHUD showSuccessWithStatus:PaxGetSuccess];
